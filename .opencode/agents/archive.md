...
- Archive 最終檔必須包含 `Bug Fix Locator Index` 或等價章節，供 archive 後的 `worktree-bug-fix` 以 `ARCHIVED_RUN_MODE` 快速定位。索引至少保留 source head、每個非 merge commit 的 commit id（完整 commit hash；active index 中等同 `final-report-index.json` 的 `commitMap[].hash`）、subject/body 摘要、標籤（規格/實作/測試/修正/文件/設定）、classification ID、OpenSpec change、touched files、source stage/worktree/branch、需求/驗收對齊、verification result 與可搜尋關鍵字（功能、API route、component/page、schema/model、錯誤訊息）。若 final maintained report 已有完整 commit map 但缺 locator index，archive agent 可在 archive 檔中追加此章節；不得發明 final report 或 git log 無法支持的內容。此 locator 只供 worktree-bug-fix 恢復/定位 final merge worktree；不得被解讀為允許 bugfix 切到或修改 init/bootstrap branch。
...
- Archive 採 summary-first 以減少 token：若 `.opencode/run-artifacts/<run_id>/final-report-index.json` 存在且 schemaVersion=`final-report-index/v1`、source final report hash、final integration head、run_id 與 archive source 一致，可用於快速檢查 commit map / Bug Fix Locator Index / touched files；此 index 的 canonical commit id 必須是 `commitMap[].hash`，`commitMapByHash` / `fileToCommits` / `classificationToCommits` 只能指向這些 hash。若 missing/stale/blocked 或與 final maintained report 不一致，必須回讀完整 final maintained report。index 不得取代 archive final file，不得取代 source head contained gate、cleanup confirmation 或 branch contained gate；也不得讓後續 bugfix 以 target bootstrap branch 作為修復目標。
