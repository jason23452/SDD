...
- `worktree-bug-fix` 已取得 Mode Selected Run Change Lock Packet，且 `ready_for_bug_triage=true`。
...
- `RUN_CHANGE_LOCK_REQUIRED`：缺少可用 Mode Selected Run Change Lock Packet，必須先由 `worktree-bug-fix` 完成 run_id lock 與 mode question。
