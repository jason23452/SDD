...
本 agent 不是整套流程終點。完成或失敗後都要把結果交還主流程；主流程預設需繼續產生/更新需求開發實踐檔，依 Stage Execution Graph 的 apply 階段、parallelGroupId、eligibleSetId 與優先度 lane 分 stage 交 `worktree-splitter` 建立目前階段 worktree，依同一 eligible set 分批執行 OpenSpec propose/spec、apply-change/fallback 與 merge integration；同一 eligibleSetId 內多個 worktree 必須由主流程同一輪平行呼叫多個 runner subagent，並以 dispatch ledger 追蹤。除非使用者主動明確限制流程，主流程下一步必須直接進入 development-detail-planner -> stage-scoped worktree-splitter，不得停下來等待使用者再次確認。
...
- 已確認 downstream：預設完整鏈路 `development-detail-planner -> stage-scoped worktree-splitter -> staged/priority-lane/parallelGroup OpenSpec propose/spec -> staged/priority-lane/parallelGroup apply-change/fallback -> stage merge integration -> final integration` 與 commit 授權狀態，或使用者主動明確限制後的有限鏈路；若缺失，不得自行補成 `bootstrap only`。
...
- 完成後輸出「回主流程續行」欄位，提供主流程產檔與後續交接需要的資料；不得要求使用者重新說明 downstream 或 commit 授權。除非使用者主動明確限制為 `bootstrap only` 或 `no commit`，續行指令必須是「主流程產生/更新 development-detail-planner 後，依 Stage Execution Graph 與 eligibleSetId 分 stage 交 worktree-splitter 建立目前 stage worktree，分批跑 OpenSpec propose/spec 與 apply/fallback；同一 eligibleSetId 內多個 worktree 由主流程同一輪平行呼叫多個 runner subagent；dispatch ledger 追蹤批次、錯誤與重試；stage merge integration 完成後才建立下一 stage worktree；apply/fallback 成功後依小功能中文細分 commit」。
...
- 續行指令：主流程產生/更新 development-detail-planner 後依 Stage Execution Graph eligible set 分 stage 建立 worktree、分批 OpenSpec propose/spec、分批 apply/fallback、stage merge integration 與 final integration；同一 eligibleSetId 內多個 worktree 由主流程同一輪平行呼叫多個 runner subagent，dispatch ledger 追蹤批次、錯誤與重試，且 apply/fallback 成功後依小功能中文細分 commit；只有使用者主動明確限制為 bootstrap only 時，才可停止於此
```
