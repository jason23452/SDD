...
- 內容只整理需求目標、使用者、情境、範圍邊界、限制、交付、驗收、既有需求關係與版本決策；不可擴寫技術方案、介面、資料模型、內部架構、資料流程、測試策略或部署方案。
...
- 若出現 `keep_old`、`needs_decision`、`conflict`、`uncertain` 或工具關卡錯誤，不可自行寫檔；回報入口代理需回到 `requirements-clarify`。
