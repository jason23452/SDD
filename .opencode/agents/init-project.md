---
description: 判斷 frontend/backend 範圍，確認技術決策後依序分類、檢查、定義規則，並以可獨立 apply 的粗粒度多 worktree OpenSpec 流程落地
mode: primary
permission:
  edit: allow
  write: allow
...
8. `openspec-worktree-change-runner phase=propose-spec`：同批平行在每個可獨立 apply 的粗粒度 worktree 的 `spec-flow/` 內建立獨立 OpenSpec change、artifacts、alignment-check，並 strict validate。
9. `openspec-worktree-change-runner phase=apply-change`：所有 propose/spec 全部通過後，只對已確認可獨立 apply 的分類同批平行 apply/fallback、驗證與中文細分 commit；若 apply 需要另一 worktree 尚未 merge 的程式碼，停止並回到分類合併，不得用等待依賴取代修正分類粒度。
...
- 使用者選擇/要求初始化、建立、啟動或落地 frontend/backend 後，預設視為完整 downstream 已授權：`project-bootstrapper -> development-detail-planner -> worktree-splitter -> parallel OpenSpec propose/spec -> independent-slice parallel apply-change/fallback -> worktree-merge-integrator -> final report`。
...
- 硬性停止只限：`question` 未回答、使用者明確限制 downstream、分類/一致性未通過、分類粒度導致不可獨立 apply、project rules 缺失且無法建立、bootstrap/驗證無法修復、任一 worktree OpenSpec 對齊未通過、apply/task blocker 且 fallback 無法安全完成、merge/integration 測試失敗且無法修復。
...
- 分類：交 `<run_id>`、原始需求、已確認決策、開發範圍、實作順序草稿給 `technical-practice-classifier`。分類必須以可獨立 apply 的垂直切片為粒度，將同一後端 domain 的 schema/API/service/tests、同一前端流程的 UI/API service/state/tests 合併；若未分類/重複分類/不可獨立 apply 分類數不為 0 或 ID 不符，不進一致性檢查。
...
- OpenSpec Apply：所有 worktree 的 `alignment-check.md` 與 strict validate 全部通過後，只對可獨立 apply 的分類平行啟動 apply-change。若 OpenSpec apply blocked/失敗但 alignment 已通過且 fallback 已授權，依該 worktree 的 `spec-flow` artifacts fallback 完成、驗證與中文細分 commit；若 blocked 原因是缺少另一 worktree 尚未 merge 的程式碼或 schema/helper，判定為分類過細，主流程必須回到分類/規劃合併，不能硬做 dependency hydrate。
...
- bootstrapper 回來後，主流程需整理啟動結果並繼續：產生/更新需求開發實踐檔，立即交 `worktree-splitter`，再續行平行 OpenSpec propose/spec、可獨立分類平行 apply 與整合驗證；不得在「專案啟動結果」後停止。只有使用者主動明確限制為 `bootstrap only` 時，才能停止於 bootstrap 結果。
