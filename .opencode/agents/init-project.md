...
2. `technical-practice-classifier`：產生互斥通用能力分類、apply 階段、優先度 lane 與執行優先度；分類 ID 固定 `<run_id>-featurs-<name>`。
...
7. `worktree-splitter`：依技術實踐分類與 apply 階段建立 `.worktree/<run_id>/stage-<n>/<name>` 或等價路徑與分支，並同步對應階段的基準快照。
8. `openspec-worktree-change-runner phase=propose-spec`：依 apply 階段拆成 `需要優先度` 與 `不需優先度` lane；兩條 lane 平行處理。`需要優先度` lane 內依數字優先度處理，同優先度同步/平行；`不需優先度` lane 內全部同步/平行。每個 worktree 在自己的 `spec-flow/` 內建立獨立 OpenSpec change、artifacts、alignment-check，並 strict validate。
9. `openspec-worktree-change-runner phase=apply-change`：同一 apply 階段的 propose/spec 全部通過後，沿用相同雙 lane 規則 apply/fallback、驗證與中文細分 commit。該階段兩條 lane 都完成後先 merge integration，再以新 integration 作為下一階段基準。若 apply 需要同階段另一 worktree 尚未 merge 的程式碼，停止並回到分類調整階段或合併分類。
...
- 使用者選擇/要求初始化、建立、啟動或落地 frontend/backend 後，預設視為完整 downstream 已授權：`project-bootstrapper -> development-detail-planner -> worktree-splitter -> staged/priority-lane OpenSpec propose/spec -> staged/priority-lane apply-change/fallback -> stage merge integration -> final integration -> final report`；同一 apply 階段內 `需要優先度` 與 `不需優先度` lane 平行處理。
...
- 分類：交 `<run_id>`、原始需求、已確認決策、開發範圍、實作順序草稿給 `technical-practice-classifier`。分類必須以通用需求能力為粒度，同類能力放同一分類，並輸出 apply 階段、優先度 lane、執行優先度與上游依賴；若未分類/重複分類/同階段阻塞依賴/循環依賴/不需優先度 lane 不可同步平行分類/無法在所列上游合併後 apply 分類數不為 0，或 ID 不符，不進一致性檢查。
...
- Worktree Split：需求開發實踐檔產生後，依 apply 階段交 `worktree-splitter` 建立 `.worktree/<run_id>/stage-<n>/<name>` 或等價路徑、branch、port map 並同步該階段基準快照。splitter 不實作、不測試、不 commit、不 merge、不 push。
...
- OpenSpec Apply：同一 apply 階段 worktree 的 `alignment-check.md` 與 strict validate 全部通過後，分成 `需要優先度` 與 `不需優先度` lane 平行啟動 apply-change；`需要優先度` lane 內依數字由小到大，`不需優先度` lane 內同步/平行。若 OpenSpec apply blocked/失敗但 alignment 已通過且 fallback 已授權，依該 worktree 的 `spec-flow` artifacts fallback 完成、驗證與中文細分 commit；若 blocked 原因是缺少同階段另一 worktree 尚未 merge 的程式碼或 schema/helper，判定為分類/階段錯誤，主流程必須回到分類/規劃調整，不能硬做 dependency hydrate。
...
- bootstrapper 回來後，主流程需整理啟動結果並繼續：產生/更新需求開發實踐檔，依分類 apply 階段、優先度 lane 與執行優先度交 `worktree-splitter`，再續行分批 OpenSpec propose/spec、分批 apply 與整合驗證；同一 apply 階段內 `需要優先度` 與 `不需優先度` lane 平行處理；不得在「專案啟動結果」後停止。只有使用者主動明確限制為 `bootstrap only` 時，才能停止於 bootstrap 結果。
