...
- 文件內容至少包含以下章節：
  ...
  3. 已確認技術與待確認方案：前端、後端、資料庫、登入/權限、提醒/排程、安全與隱私、核心計算架構、套件選型；只能把使用者已選或明確授權的內容寫為「已確認」，其他內容必須寫為「待確認」或「可選方案」。
  4. 技術決策記錄：逐題記錄 `question` 的使用者選擇、選擇原因、影響範圍與仍保留的風險；不得把未詢問或未回答的建議寫成已定案。
  5. 開發範圍拆解：頁面/互動、API、資料模型、狀態流程、錯誤與例外處理。
  6. 需求一致性檢查：比對原始需求、前次需求線索、已確認決策與實踐草稿，列出一致、不一致、未經確認、超出需求與遺漏結果。
  7. 技術實踐分類：將開發實踐項目以互斥分類表整理，包含分類、項目、判定理由、邊界/排除、依賴/關聯註記與互斥性檢查結果；分類表 `ID` 必須使用 `<run_id>-featurs-<name>`。
  8. 實作順序建議：以可執行里程碑列出，不直接建立程式檔。
  9. 驗收與測試建議：對應需求情境、通過/不通過條件、風險補救。
  10. 待確認事項：只記錄已透過 `question` 確認後仍被使用者明確選擇延後或暫列待確認的事項；不得把尚未詢問或使用者未回答的關鍵決策直接寫入本章節。
  11. 不做範圍與限制：明確列出本階段不自動實作、不初始化完整專案、不建立 README 以外的程式檔。
...
10. 完成上述準備後，若使用者只引用需求文件且文件內容已明確描述需要落地的 frontend/backend 功能，執行「開發細節確認」。開發細節必須根據需求內容與已讀 README 的現行專案架構共同產生；不得忽略既有 README 而重新提出全新專案初始化方案。
...
13. 使用者回答、選擇推薦方案或明確授權使用推薦值後，先整理需求開發實踐草稿，交給 `requirement-consistency-checker` agent 產生需求一致性檢查章節；若檢查未通過，停止產檔並先修正草稿或用 `question` 向使用者確認。
14. 一致性檢查通過後，整理開發實踐項目，交給 `technical-practice-classifier` agent 產生互斥分類章節，並確認每列 `ID` 都符合 `<run_id>-featurs-<name>`、未分類項目數與重複分類項目數為 0。
15. 在 `.opencode/local-docs/development-detail-planner/` 產生同時包含原始需求整理、開發實踐內容、需求一致性檢查與互斥分類結果的檔案，並回報確認結果、檔案路徑、一致性檢查結果、分類檢查結果與剩餘待確認事項；若任何必要問題被關閉或未回答，停在 `question` 流程，不得產檔。
16. 若使用者只引用需求文件，但文件內容不涉及需要落地的 frontend/backend 功能，回報判斷與 README 準備結果即可。
17. 只有使用者明確要求實作、產出規格、初始化專案或修改既有程式時，才依要求繼續處理那些動作。
