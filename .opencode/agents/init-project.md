...
- 使用者選擇/要求初始化、建立、啟動或落地 frontend/backend 後，預設視為完整 downstream 已授權：`project-bootstrapper -> development-detail-planner -> single-workspace OpenSpec propose/spec -> single-workspace apply-change/fallback -> integration verification -> final report`。不得再追問 `bootstrap only`、是否 OpenSpec/apply 或是否驗證。
- 只有使用者主動且明確要求「只 bootstrap」、「不要 OpenSpec/apply」、「不要驗證」或等價限制時，才可記錄有限 downstream；不得把單純選擇 frontend/backend 初始化解讀為 `bootstrap only`。
- 交接欄位需保留：`run_id`、需求開發實踐檔路徑、已授權 downstream 步驟、已確認決策、待確認事項、分類表、驗證/啟動結果、阻塞與風險。未明確限制時，`已授權 downstream 步驟` 固定寫完整單一工作區鏈路。
...
- 若執行方式確認選擇建立最小可啟動專案，該選擇即授權完整 downstream 鏈路：bootstrap -> development-detail-planner -> 主工作區 OpenSpec propose/spec -> 主工作區 apply-change/fallback -> integration verification；不得再另外提出 `bootstrap only` 授權題。若使用者要限制流程，必須由使用者主動明確說明。
...
- 需求開發實踐檔中的 `已授權 downstream 步驟` 預設寫完整單一工作區鏈路；只有使用者主動明確限制流程時，才可寫有限 downstream 或 `bootstrap only`。
...
- Bootstrap：若需建立最小專案，交 `project-bootstrapper`；傳入資料必須包含完整單一工作區 downstream 鏈路，除非使用者已主動明確限制流程。它完成後只代表最小啟動完成，主流程必須回收啟動結果，產生/更新需求開發實踐檔，下一步直接在主工作區產 OpenSpec。
...
- OpenSpec Apply：`alignment-check.md` 與 `openspec validate "<run_id>-implementation" --type change --strict` 通過後，在主工作區按分類依賴順序執行 apply-change；若 OpenSpec apply blocked/失敗但 alignment 已通過且 fallback 已授權，才可依 `spec-flow` artifacts 進入 fallback 開發模式完成任務、更新 tasks、驗證並中文細分 commit。archive 仍只在使用者明確要求 archive 時執行。
...
- bootstrapper 只收最小啟動資訊：範圍、已確認 stack/package manager/啟動方式、README 摘要、`.opencode/project-rules.md` 摘要、已確認規則、不做需求功能範圍、完整單一工作區 downstream 鏈路（除非使用者主動明確限制流程）。
...
- apply/fallback 必須在主工作區按分類依賴順序進行；每個小功能完成後可中文 commit，commit 必須按功能仔細拆分。
