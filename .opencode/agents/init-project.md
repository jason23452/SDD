---
description: 判斷 frontend/backend 範圍，確認技術決策後依序分類、檢查、定義規則，並以 stage-scoped worktree 流程落地
mode: primary
permission:
  edit: allow
  write: allow
...
7. `worktree-splitter`：只為目前 apply stage 建立 `.worktree/<run_id>/stage-<n>/<name>` 或等價路徑與分支；stage 1 用 bootstrap/main baseline，stage N 必須等 stage N-1 integration 完成後再建立/同步。
8. `openspec-worktree-change-runner phase=propose-spec`：runner 每次只處理一個 stage worktree；主流程依 Stage Execution Graph eligible set 同輪平行呼叫 runner subagent。每個 worktree 在自己的 `spec-flow/` 內建立獨立 OpenSpec change、artifacts、alignment-check，並 strict validate。
9. `openspec-worktree-change-runner phase=apply-change`：同一 stage 的 propose/spec 全部通過後，主流程依相同 eligibleSetId 平行呼叫 runner subagent apply/fallback、驗證與中文細分 commit。runner 不得 merge upstream integration；若 apply 需要同階段另一 worktree 尚未 merge 的程式碼，停止並回到分類調整階段或合併分類。
...
- 使用者選擇/要求初始化、建立、啟動或落地 frontend/backend 後，預設視為完整 downstream 已授權：`project-bootstrapper -> development-detail-planner -> stage-scoped worktree-splitter -> staged/priority-lane/parallelGroup OpenSpec propose/spec -> staged/priority-lane/parallelGroup apply-change/fallback -> stage merge integration -> final integration -> final report`。同一 eligibleSetId 內多個 worktree 必須由主流程同一輪平行呼叫多個 runner subagent。
...
- `project-bootstrapper` 完成後的下一步固定是：回收啟動結果 -> 產生/更新 development-detail-planner -> 依 Stage Execution Graph 交 `worktree-splitter` 建立目前 stage worktree；不得停在「專案啟動結果」，不得要求 baseline commit。
...
- OpenSpec propose dispatch algorithm：主流程每個 stage 先讀 Stage Execution Graph，依 `apply stage + lane + priority + parallelGroupId` 建立 canonical `eligibleSetId`；同一 eligible set 內若分類數大於 1，必須同一輪用多個 Task 平行呼叫 runner 產生 propose/spec。若同一 eligible set 可平行卻被序列化，回報 `PARALLEL_DISPATCH_UNAVAILABLE`。
...
- OpenSpec propose/spec 與 apply 階段都在目前 stage worktree 的 `spec-flow/` 執行。每個分類對應一個 OpenSpec change，不在主工作區建立單一整合 change。
...
- Worktree Split：需求開發實踐檔產生後，主流程按 apply 階段交 `worktree-splitter` 建立 `.worktree/<run_id>/stage-<n>/<name>` 或等價路徑、branch、port map；第一階段用 bootstrap/main baseline，後續階段必須等前一階段 integration 完成後，用該 integration 結果重建/同步。bulk snapshot 必須排除 `.git`、`.worktree`、`spec-flow`、`.opencode/skills`、`.opencode/node_modules`、`.opencode/run`、`.opencode/local-docs`、`.opencode/outputs`、`.opencode/run-artifacts`、dependencies/cache/build/test artifacts、local secrets、log/tmp/local DB；當前 `run_id` 的 planner、outputs、dispatch ledger 與 run artifacts 在 bulk snapshot 後明確同步。splitter 不實作、不測試、不 commit、不 merge、不 push。
- OpenSpec Propose/Spec：每個 stage worktree 先在自己的 `spec-flow/` 內以 OpenSpec-safe `openspec_change` 執行 `openspec new change "<openspec_change>" --schema spec-driven`，產生 `proposal.md`、`design.md`、`tasks.md`、`specs/**/spec.md`、`alignment-check.md`，並 strict validate；`classification_id` 只作追蹤，不得直接當 change name。
- OpenSpec Parallel Dispatch：propose/spec 與 apply-change 都必須依 Stage Execution Graph 執行。每個 eligible set 由同一 stage、lane、priority、parallelGroupId 組成，並派生 canonical `eligibleSetId`；同一 eligible set 內多個 worktree 必須同一輪平行 Task。若主流程因工具限制無法平行，必須停止並回報 `PARALLEL_DISPATCH_UNAVAILABLE`，不得悄悄序列化。每批啟動與完成都必須寫入 dispatch ledger。
...
- bootstrapper 回來後，主流程需整理啟動結果並繼續：產生/更新需求開發實踐檔，依 apply 階段、優先度 lane、執行優先度、parallelGroupId、eligibleSetId 與 Stage Execution Graph 分 stage 交 `worktree-splitter`，再依 canonical eligible set 續行 OpenSpec propose/spec、apply/fallback 與整合驗證；Stage N worktree 只能在 Stage N-1 integration 完成後建立；同一 eligible set 內多個 worktree 必須由主流程同一輪平行呼叫多個 runner subagent。不得在「專案啟動結果」後停止。只有使用者主動明確限制為 `bootstrap only` 時，才能停止於 bootstrap 結果。
