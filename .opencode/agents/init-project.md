...
8. `openspec-worktree-change-runner phase=propose-spec`：runner 每次只處理一個 worktree；主流程依 Stage Execution Graph 建立 eligible set（`apply stage + lane + priority + parallelGroupId`），同一 eligible set 內多個 worktree 必須同一輪平行呼叫多個 runner subagent。每個 worktree 在自己的 `spec-flow/` 內建立獨立 OpenSpec change、artifacts、alignment-check，並 strict validate。
9. `openspec-worktree-change-runner phase=apply-change`：同一 apply 階段的 propose/spec 全部通過後，主流程沿用相同 eligible set 規則平行呼叫多個 runner subagent apply/fallback、驗證與中文細分 commit。該階段兩條 lane 都完成後先 merge integration，再以新 integration 作為下一階段 splitter 基準。runner 不得 merge upstream integration；若 apply 需要同階段另一 worktree 尚未 merge 的程式碼，停止並回到分類調整階段或合併分類。
...
- `project-bootstrapper` 完成後的下一步固定是：回收啟動結果 -> 產生/更新 development-detail-planner -> 依分類 apply 階段、priority、parallelGroupId、eligibleSetId 與 Stage Execution Graph 交 `worktree-splitter`；不得停在「專案啟動結果」，不得要求 baseline commit。
...
- Worktree Split：需求開發實踐檔產生後，主流程按 apply 階段分批交 `worktree-splitter` 建立 `.worktree/<run_id>/stage-<n>/<name>` 或等價路徑、branch、port map，並從目前 SDD repository root bulk copy 可用開發 snapshot 到每個 worktree。bulk snapshot 必須排除 `.git`、`.worktree`、`spec-flow`、`.opencode/skills`、`.opencode/node_modules`、`.opencode/run`、`.opencode/local-docs`、`.opencode/outputs`、`.opencode/run-artifacts`、dependencies/cache/build/test artifacts、local secrets、log/tmp/local DB；當前 `run_id` 的 planner、outputs、dispatch ledger 與 run artifacts 在 bulk snapshot 後明確同步。第一階段用 bootstrap/main baseline；後續階段必須等前一階段 integration 完成後，用該 integration 結果重建/同步。splitter 不實作、不測試、不 commit、不 merge、不 push。
...
- bootstrapper 回來後，主流程需整理啟動結果並繼續：產生/更新需求開發實踐檔，依分類 apply 階段、優先度 lane、執行優先度、parallelGroupId、eligibleSetId 與 Stage Execution Graph 分 stage 交 `worktree-splitter`，再依 canonical eligible set 續行分批 OpenSpec propose/spec、分批 apply 與整合驗證；同一 eligible set 內多個 worktree 必須由主流程同一輪平行呼叫多個 runner subagent。不得在「專案啟動結果」後停止。只有使用者主動明確限制為 `bootstrap only` 時，才能停止於 bootstrap 結果。
