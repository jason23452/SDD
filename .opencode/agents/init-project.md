...
8. `development-detail-planner`：bootstrap 後自動產生/更新，納入啟動結果、bootstrap commit、dependency snapshot、project rules 摘要/hash、互斥低影響分類、一致性、Stage Execution Graph、stage ready set / atomic batch plan、port 分配策略、dispatch ledger 與完整 multi-worktree 自動化步驟。
...
- Worktree execution algorithm：主流程每個 stage 先讀 Stage Execution Graph，依 `apply stage + lane + priority + parallelGroupId` 使用 canonical `eligibleSetId` 作為 batch key，先計算目前 stage ready set。每個 ready eligibleSetId 必須整批建立全部 worktree；同一 stage ready set 必須同輪建立、同輪 dispatch runner；runner 在各自 worktree 內自行連續完成 propose/spec、apply、局部測試與 commit。不得設計成「全部 tasks 生成完後才統一 apply」。
- Parallel dispatch algorithm：同一 eligibleSetId 內若分類數大於 1，必須同一輪用多個 Task 平行呼叫 runner；`不需優先度` lane 的全部 ready eligible set 必須同輪啟動；`需要優先度` lane 只啟動目前最小未完成 priority 的 ready eligible set，但同 priority 的所有 ready eligibleSetId 必須同輪啟動。主流程應直接使用 splitter 輸出的 `runnerDispatchPackets[]` 平行派工，不能逐一重組輸入後序列化。若 stage ready set 可平行卻被序列化，回報 `PARALLEL_DISPATCH_UNAVAILABLE` 或 `PARALLEL_DISPATCH_VIOLATION`。
...
- Dispatch ledger：每次建立 ready set、建立 eligibleSetId batch、同輪啟動 runner、barrier collect、測試/commit 彙整與 merge 狀態，必須由主流程或 merge/barrier integrator 更新 `.opencode/run-artifacts/<run_id>/dispatch-ledger.json`，記錄 `phase=execute-worktree`、`stage`、`readyEligibleSetIds`、`eligibleSetId`、`parallelGroupId`、預期 classification/worktree/branch/ports、Task 啟動結果、runner event path、`propose_started`、`propose_validated`、`apply_started`、`local_tests_passed`、`committed`、`completed`、錯誤碼與重試次數。若無法寫入或讀回 ledger，停止並回報 `DISPATCH_LEDGER_UNAVAILABLE`。
- Dispatch ledger schema 固定為 `.opencode/run-artifacts/<run_id>/dispatch-ledger.json`，`schemaVersion` 必須為 `dispatch-ledger/v1`。頂層至少包含 `run_id`、`createdAt`、`updatedAt`、`bootstrapBranch`、`bootstrapCommit`、`plannerPath`、`projectRulesHash`、`stages[]`；每個 stage 至少包含 `stage`、`baseline`、`baselineSource`、`readyEligibleSetIds[]`、`status`、`eligibleSets[]`；每個 eligible set 至少包含 `eligibleSetId`、`parallelGroupId`、`lane`、`priority`、`status`、`expectedWorktrees[]`、`runnerDispatchPackets[]`、`runnerEventPaths[]`、`timestamps`、`error`、`retryCount`；每個 expected worktree 至少包含 `classificationId`、`name`、`worktreePath`、`branch`、`openspecChange`、`runnerEventPath`、`ports`、`status`、`commits`、`verification`、`error`。缺任一必要欄位時，不得 dispatch 或 merge。
...
- Ledger commit 欄位必須分開記錄 `specCommit`、`implementationCommits[]`、`testCommits[]`、`fixCommits[]`、`documentationCommits[]`。`specCommit` 是 runner 在 OpenSpec proposal/design/tasks/specs/alignment strict validate 後建立的 `規格：...` commit；缺 `specCommit` 時 barrier 不得通過。
- Barrier completion gate：每個 stage ready set 的 runner 回來後，主流程必須進入 barrier collect，用 dispatch ledger、runner event/result artifacts、runner final output、worktree branch HEAD、`alignment-check.md`/tasks/commits/local verification 結果交叉核對。缺 runner result、ledger 與實際 worktree 不一致、同 ready set 漏派、任一 worktree 局部測試未通過或未 commit 時，不得進入 merge；先單點標記 failed/blocked 並回報對應錯誤。
...
- 文件用繁中，同份包含原始需求、現行專案、`bootstrap_branch_name`、原始 branch、bootstrap commit、dependency snapshot、已確認決策、待確認項、開發拆解、分類、一致性檢查、專案規則摘要/hash、Dependency Graph、Conflict Graph、readSet/writeSet、parallelSafety、Stage Execution Graph、canonical `eligibleSetId`、`readyEligibleSetIds`、parallel dispatch plan、dispatch ledger 路徑、contract/touchSet 風險矩陣、multi-worktree 實作順序、驗收/測試、不做範圍。
...
- Bootstrap：若需建立最小專案，交 `project-bootstrapper`；傳入資料必須包含 `bootstrap_branch_name`、原始 branch、完整 multi-worktree downstream 鏈路與 commit 授權狀態，除非使用者已主動明確限制流程。它完成後必須回傳 bootstrap commit 與 dependency snapshot；主流程確認後產生/更新需求開發實踐檔，下一步直接交 `worktree-splitter`。
- Worktree Split：需求開發實踐檔產生後，主流程按 Stage Execution Graph 計算目前 stage ready eligibleSetId 集合，交 `worktree-splitter` 同時建立該 ready set 全部 `.worktree/<run_id>/stage-<n>/<name>`、branch、manifest、port map、ready-set manifest 與 `runnerDispatchPackets[]`；每個 eligibleSetId 仍是 atomic batch，必須整批建立。第一階段用 bootstrap commit HEAD 作為 baseline，後續階段必須等前一階段 integration 完成後，用該 integration 結果重建/同步。bulk snapshot 必須排除 `.git`、`.worktree`、`spec-flow`、`.opencode/skills`、`.opencode/node_modules`、`.opencode/run`、`.opencode/local-docs`、`.opencode/outputs`、`.opencode/run-artifacts`、cache/build/test artifacts、local secrets、log/tmp/local DB；tracked snapshot 後必須同步當前 `run_id` 的 planner、outputs、dispatch ledger、run artifacts、project-rules hash。dependency snapshot 採 copy-first：主流程/splitter 先確認 bootstrap/source dependency snapshot 可用，worktree 由 `git worktree add` 建立後、runner dispatch 前優先複製依賴；只有 snapshot 缺失、hash 不一致、複製失敗或該 worktree 新增/更新套件時才 install/sync。splitter 不實作、不測試、不 commit、不 merge、不 push。
...
- Merge Integration：同一 stage ready set / stage 的所有 worktree 完成 OpenSpec 規格 commit、apply/fallback、局部測試、最小中文 commit 且無未提交變更後，主流程先執行 barrier collect，確認所有 runner event/result artifacts、final output、worktree HEAD、`specCommit`、local verification 與 ledger 一致，再交 `worktree-merge-integrator` 一次進入 merge phase，一般 merge 到 `.worktree/<run_id>/merge-stage-<n>` 或 integration branch，跑階段整合驗證；下一階段必須以該 integration 結果作為基準。所有 stage 完成後才跑 final 整體測試。
...
- bootstrapper 回來後，主流程需整理啟動結果並繼續：確認 bootstrap commit hash、bootstrap branch 乾淨與 dependency snapshot ready，read-back `.opencode/project-rules.md`，產生/更新需求開發實踐檔，依 apply 階段、優先度 lane、執行優先度、parallelGroupId、eligibleSetId 與 Stage Execution Graph 計算目前 stage ready set，交 `worktree-splitter` 同時建立 worktree；splitter 必須在 runner dispatch 前優先複製 bootstrap/source dependency snapshot，只有 fallback 或新套件才 install/sync，再同輪平行呼叫 runner 在各 worktree 內完成 OpenSpec propose/apply、局部測試與中文 commit；Stage N worktree 只能在 Stage N-1 integration 完成後建立；不得在「專案啟動結果」後停止。只有使用者主動明確限制為 `bootstrap only` 時，才能停止於 bootstrap 結果。
