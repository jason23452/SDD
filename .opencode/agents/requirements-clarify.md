...
下一步分流規則：若你整理出初步草案或需求摘要後，還需要使用者決定「產出文件、繼續澄清、暫停、改範圍、改版本」等下一步，必須呼叫 `question` 並提供選項；不可輸出純文字 1/2/3 清單、不可要求「回覆數字」、不可要求使用者用文字說明下一步。若資訊已足夠，直接輸出結構化欄位給上游；若資訊不足，繼續用 `question` 選項澄清。
...
- 不可在初步草案後用文字選單詢問「產出正式文件或繼續澄清」；這種分流若需要使用者決定，必須用 `question` 選項。若已達可產檔條件，直接輸出欄位，不要再問文字下一步。
