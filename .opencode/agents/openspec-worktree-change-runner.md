...
- 輸入應含 `run_id`、`classification_id`、`apply_stage`、`ready_wave_id`、`execution_lane`、`execution_priority`、`parallelGroupId`、`eligibleSetId`、`ownerCapability`、`ownedRequirements`、`excludedResponsibilities`、`touchSet`、`contractInputs`、`contractOutputs`、`testImpact`、`impactReason`、`isolationStrategy`、`conflictRisk`、`packageNeeds`、`packageOwner`、`packageDecisionRecordRef`、`manualBuildReason`、`activeSkills`、`upstream_dependencies`、`worktree`、`branch`、`spec_flow_path`、`openspec_change`、dispatch ledger 路徑、development-detail-planner 路徑、planner path in worktree、project rules path/hash、run artifacts path、copied_files、dependency snapshot manifest、dependency snapshot、技術實踐項目、Experience Contract、Package Decision Record、已確認決策、不做範圍、驗證需求、ports、fallback 是否授權與 commit 授權狀態。若 development-detail-planner 路徑缺失，或該檔不在 worktree 內，必須依「Run Artifacts 與 Planner 解析契約」自動解析；不得在尚未嘗試 manifest、port-map 與主工作區 fallback 前要求使用者補路徑。若 `openspec_change` 缺失或不合法，依「OpenSpec Change Name 契約」自動派生合法名稱，不得直接使用 `classification_id`。
...
- 本 worktree branch 必須 exactly 符合 `worktree/<run_id>/stage-<n>/<name>`。若輸入 packet、manifest、dispatch ledger、runner event path 對應資料或目前 git branch 使用 `work/<run_id>/*`、其他 alias，或與 `worktree/<run_id>/*` 不一致，停止回報 `WORKTREE_BRANCH_NAMESPACE_INVALID`；runner 不得自行改寫 branch、不得把 alias 視為相容。
...
OpenSpec artifacts 通過 strict validate 與 alignment 後，runner 可寫入 `.opencode/run-artifacts/<run_id>/openspec-change-index/<classification_id>.json`，schemaVersion=`openspec-change-index/v1`，保存 proposal/design/tasks/spec paths/hash、specs hash、validate result、alignment result、task count、done count、specCommit 與 blockers。此 index 只供 barrier/merge 減少讀全文；若 missing/stale/blocked 或與 git HEAD/specCommit 不一致，barrier/merge 必須回讀完整 OpenSpec artifacts。
...
Package-first gate 是 propose、apply、dependency sync、verification 與 commit 前的硬性 gate：

1. 必須讀取 development-detail-planner 的 active skills、Experience Contract 與 Package Decision Record，確認本 classification 的 `packageNeeds`、`packageOwner`、`packageDecisionRecordRef`、`manualBuildReason` 與輸入一致。
2. 若本 classification 涉及成熟套件適用能力，例如表單驗證、日期/日曆、表格/圖表、拖曳、UI primitives、server-state、auth/JWT/session、password hashing、ORM/migration、cache/Redis、queue/scheduler、外部 HTTP client、file upload、i18n、logging/observability、retry/timeout，但 Package Decision Record 沒有 selected package 或 manual-build reason，停止回報 `PACKAGE_DECISION_REQUIRED`。
3. 只能安裝或使用 Package Decision Record 中已確認、既有專案已存在，或 active skill 明確要求且已由主流程確認的套件。若實作中發現需要新套件，停止並回報 `PACKAGE_DECISION_REQUIRED`，不得自行安裝後補記錄。
4. 套件使用不得違反 active skill，而非套用未觸發 skill 的規則。若 active skill 與 planner/package decision 不一致，停止回報 `PROJECT_RULES_ALIGNMENT_FAILED` 或 `PACKAGE_DECISION_REQUIRED`。
5. 若選擇手刻成熟能力，必須在 proposal、design、tasks、commit body 與 runner event 記錄 manual-build reason、風險與驗證；缺理由不得進入 apply。
6. 新增或修改套件 manifest/lockfile 後，必須使用既有 package manager 同步 dependency，且只提交 manifest/lockfile 與必要 source/test/config，不提交 dependency directory。
...
- 本 worktree 只能使用本分類 ownership 內的 package decisions；不得導入其他分類、foundation 或全域 infrastructure owner 的未合併套件與 contract。
...
- 若存在 `.opencode/run-artifacts/<run_id>/port-registry.json`，runner 可讀取它作為 lifecycle 參考，但不得直接寫入 shared port registry。runner 若啟動或使用任何 assigned port，必須只在自己的 `runner_event_path` 記錄 `portUsage[]`（portName、port、startedByRun、pid/handle/composeProject、cleanupState、blocker），由主流程或 merge/barrier integrator 單點彙整。
...
- shared dispatch ledger 與 manifest 中的 `branch` 必須等於目前 git branch，且必須以 `worktree/<run_id>/` 開頭；若不符合，停止回報 `WORKTREE_BRANCH_NAMESPACE_INVALID`。
...
- active skills、Experience Contract、Package Decision Record、package-first/manual-build reason 與本 worktree 套件使用是否一致。
...
- package-first gate：通過/blocked；blocker 可為 `PACKAGE_DECISION_REQUIRED`
...
### Compact Context
- context refs：passed/blocked；缺失或 stale refs：...
- context slice：classification-only/insufficient；fallback full reads：...
- compact output：enabled；status：completed/blocked/failed；blockers：無/列表；commits：spec/implementation/test/fix/documentation；verification：local commands + verification-summary refs；contextRefs：manifest/dispatch packet/project-rules-lock/dependency-readiness；artifactRefs：runner event/openspec-change-index/verification-summary；nextAction：barrier collect 或 blocker fix；fallbackUsed：none/full planner/full rules/full logs
...
預設輸出使用 compact output，避免重貼完整 planner、完整 project rules、完整 logs 或完整 specs；只列 status、blockers、artifact path/hash、commits、verification-summary、context refs、nextAction 與 fallbackUsed。若 blocked/failed，需補足最小診斷內容與可讀 logRef；若 final report 或 merge barrier 要求完整內容，才輸出對應詳細表格。
