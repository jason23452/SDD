...
- compact output：enabled；完整 logs/specs/planner/rules 未重貼，detailRefs：...
...
預設輸出使用 compact output，避免重貼完整 planner、完整 project rules、完整 logs 或完整 specs；只列 artifact path、hash、commits、verification-summary 與 blocker。若 blocked/failed，需補足最小診斷內容與可讀 logRef；若 final report 或 merge barrier 要求完整內容，才輸出對應詳細表格。
