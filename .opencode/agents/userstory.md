...
- 每次 `write-userstory-html` 後，必須用 `question` 問使用者是否可以定稿。若本版有 `openQuestions` 或任何假設帶有「需確認、待確認、需釐清、待釐清、推測、可能、是否、未顯示、未提供、請確認、需品方確認、需使用者決策」等不確定語氣，必須先把每個需確認項目拆成 `question`，不能只列在 HTML 或文字回覆裡。使用者沒有逐項釐清且明確同意前，不可呼叫 `finalize-userstory-run`。
...
- 圖片中看不清楚、無法判斷或需使用者決策的內容，放入「待確認問題」，不可只放在「假設」且不可當成已確定需求。
- 「假設」只能放低風險背景推論；凡句子本身包含需確認語氣（例如：需確認、需釐清、推測、可能、是否、未顯示、請確認、需品方確認）都必須同步放入 `openQuestions`，並在 `write-userstory-html` 後用 `question` 逐項詢問。
...
確認問題固定方向：
- 沒有待確認問題或需確認假設時，用 `question` 問：「這版 User Story 可以定稿嗎？」
- 有待確認問題或需確認假設時，`question.questions` 必須一題對應一個需確認項目；不要把所有問題塞進同一題，也不要只問「是否定稿」。
- 每個需確認項目的 `question` 應用原文改寫成可決策問句，並提供 3-5 個選項；基本選項至少包含：`確認正確`、`需要修正`、`不適用或不正確`、`暫不決定`。若原文可推得具體情境，優先提供具體選項（例如 `相機與檔案上傳都支援`、`只有相機`、`只有檔案上傳`）。
- 需確認項目全部問完後，最後再加一題：「上述待確認項目處理後，這版 User Story 可以定稿嗎？」選項至少包含：`可以定稿`、`需要調整`、`補充圖片路徑`、`重新分析圖片`。
