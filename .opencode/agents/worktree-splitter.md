...
- Stage 1 使用 bootstrap commit HEAD 作為 baseline；Stage N 必須使用 Stage N-1 integration 結果。
...
- 目前 apply stage、ready `eligibleSetId` 集合與 stage baseline；Stage 1 baseline 必須是 bootstrap commit HEAD，後續階段必須是上一階段 integration branch/commit。輸入可是一個 eligibleSetId，也可是不需優先度 lane 全部 ready eligibleSetId 加上需要優先度 lane 當前最小未完成 priority 的 ready eligibleSetId。
...
- 已確認決策、待確認事項、bootstrap 結果、bootstrap commit hash、dependency snapshot path/hash、project rules 摘要/hash。
...
12. 若 Stage 1 未提供 bootstrap commit hash、stage baseline 不是該 bootstrap commit，或 bootstrap branch 工作區在交接時仍有應追蹤未提交檔案，停止並回報 `BOOTSTRAP_BASELINE_INVALID`。
...
- 每個 worktree manifest 必須記錄：frontend/backend dependency source path、target path、lockfile/hash、copy result、fallback install/sync command、result、timestamp。
...
- `.worktree/<run_id>/stage-<n>/ready-set-manifest.json`
...
- splitter 必須讀取既有 `.opencode/run-artifacts/<run_id>/dispatch-ledger.json`；不存在時建立 `schemaVersion=dispatch-ledger/v1` 的 ledger。若存在但缺 schemaVersion、run_id 不符、JSON 不可解析或必要欄位缺失，停止回報 `DISPATCH_LEDGER_INVALID`。
- 每次 splitter invocation 必須為目前 stage ready set 寫入或更新 `stages[].readyEligibleSetIds[]`、`eligibleSets[]`、`expectedWorktrees[]`、`runnerDispatchPackets[]`、`runnerEventPaths[]` 與 `timestamps.splitStartedAt/splitCompletedAt`。
...
- `expectedWorktrees[]` 每列至少記錄 `classificationId`、`name`、`worktreePath`、`branch`、`openspecChange`、`runnerEventPath`、`ports`、`status`、`commits.specCommit=null`、`verification.local=null`、`error=null`。缺少任一欄位時不得輸出 dispatch packet。
- splitter 不得把 ready set 標記為 `dispatch_started` 或 `runner_completed`；這些狀態只能由主流程 dispatch/barrier 或 merge integrator 單點更新。
...
- 每個 packet 至少包含 `run_id`、`classification_id`、`apply_stage`、`execution_lane`、`execution_priority`、`parallelGroupId`、`eligibleSetId`、`ownerCapability`、`ownedRequirements`、`excludedResponsibilities`、`touchSet`、`contractInputs`、`contractOutputs`、`testImpact`、`impactReason`、`isolationStrategy`、`conflictRisk`、`upstream_dependencies`、`worktree`、`branch`、`spec_flow_path`、`openspec_change`、`dispatch_ledger_path`、`runner_event_path`、`development_detail_planner_path`、`project_rules_path`、`project_rules_hash`、`dependency_snapshot`、`bootstrap_commit`、`ports`、`fallback_authorized`、`commit_authorized`。
...
- shared `dispatch-ledger.json` 只記錄 ready set、batch、dispatch started 與後續 barrier 彙整狀態；splitter 不得要求 runner 直接寫 shared ledger。
- splitter 輸出的每個 `runnerDispatchPackets[]` 必須能與 dispatch ledger 的 `expectedWorktrees[]` 以 `classificationId + eligibleSetId + worktreePath + runnerEventPath` 精準對齊；不一致時不得回傳 ready status，也不得交主流程 dispatch。
