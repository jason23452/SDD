...
- run-preflight-packet、verification-matrix、package-decision-record、experience-contract 與 context-slice refs；若缺失或 stale，splitter 必須回完整 planner/project rules gate 產生等價 slice，不得讓 runner 猜測。
...
- bulk snapshot 不得預設複製整個 root；必須先由 snapshot-manifest 判斷本 run 需要的 tracked files、active project dirs、shared contracts、root config 與需求來源。大型 docs、未引用 assets、test reports、local generated files 與非本 run 需要內容應記為 skipped，除非 planner/contextRefs 明確要求。
...
- Splitter 對同一 ready wave 只做一次 root snapshot plan、project-rules hash、dependency source readiness、port allocation、package owner map 與 verification matrix slicing；每個 worktree 只取得自己的 `context-slice/v1`。不得為每個 worktree 重複推理完整 Stage Graph、Package Decision Record 或 Experience Contract。
...
- `.opencode/run-artifacts/<run_id>/skill-lock.json`（schemaVersion=`skill-lock/v1`，若存在可作 skill immutable 快速檢查來源；實際 skill diff 仍是 blocker 依據）
- `.opencode/run-artifacts/<run_id>/dependency-readiness.json`（schemaVersion=`dependency-readiness/v1`，若存在可作 dependency snapshot/readiness 快速來源；manifest/lockfile hash 不一致時回完整 Dependency Snapshot 同步規則）
- `.opencode/run-artifacts/<run_id>/resume-cursor.json`（schemaVersion=`resume-cursor/v1`，splitter/主流程可更新目前 stage ready wave 與 nextAction；cursor stale 時以 dispatch ledger 為準）
- `.opencode/run-artifacts/<run_id>/run-preflight-packet.json`（schemaVersion=`run-preflight-packet/v1`，若存在且 hash/sourceHead/status 通過，作為 ready wave 快速交接來源；stale 時回完整 planner gate）
- `.opencode/run-artifacts/<run_id>/verification-matrix.json`（schemaVersion=`verification-matrix/v1`，用於切出 runner/stage/final 驗證 slice；不得取代實際驗證命令結果）
- `.opencode/run-artifacts/<run_id>/context-slices/<classification_id>.json`（schemaVersion=`context-slice/v1`，每個 runner 的最小必要 context）
- `.opencode/run-artifacts/<run_id>/snapshot-manifest.json`（schemaVersion=`snapshot-manifest/v1`，同一 ready wave 的增量 snapshot 計畫與結果；不得取代 git worktree baseline 或 dependency snapshot manifest）

若 shared ledger 尚未存在，splitter 可用 `node .opencode/scripts/build-dispatch-ledger-skeleton.js <run_id> --planner <path> --stage <n>` 初始化骨架，再補齊 readyWave/eligibleSet/expectedWorktrees；dispatch 前必須用 `node .opencode/scripts/check-dispatch-ledger-readiness.js <run_id>` 與完整 ledger gate 檢查，不得只靠 skeleton dispatch。
...
- packet 的 `contextRefs[]` 應包含 skill-lock、dependency-readiness 與 resume-cursor refs（若存在）；任一 ref stale 只能觸發 fallback，不得阻止已具備完整來源的正常流程。
