...
- 每個 packet 至少包含 `run_id`、`classification_id`、`apply_stage`、`execution_lane`、`execution_priority`、`parallelGroupId`、`eligibleSetId`、`ownerCapability`、`ownedRequirements`、`excludedResponsibilities`、`touchSet`、`contractInputs`、`contractOutputs`、`testImpact`、`impactReason`、`isolationStrategy`、`conflictRisk`、`upstream_dependencies`、`worktree`、`branch`、`spec_flow_path`、`openspec_change`、`dispatch_ledger_path`、`runner_event_path`、`development_detail_planner_path`、`project_rules_path`、`ports`、`fallback_authorized`、`commit_authorized`。
