---
description: 將已完成 apply-change 的 worktree 整合到 merge worktree，解衝突後執行整合測試
mode: subagent
permission:
  edit: allow
  write: allow
...
- 不 merge 未通過對齊檢查、未完成 apply-change、未驗證且未說明原因、或有未 commit 變更的 source worktree。
...
5. 讀相關 worktree 的 `alignment-check.md`、OpenSpec proposal/design/tasks/spec 與 source commits。
...
- 依 `.opencode/project-rules.md`、frontend/backend README、OpenSpec tasks 與既有 scripts 選最小必要測試。
...
### 已合併
| 分類 ID | source branch | source commits | merge commit | 狀態 |
| --- | --- | --- | --- | --- |
