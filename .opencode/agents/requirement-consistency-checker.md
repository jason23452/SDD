...
- 草稿/流程若使用 `project-rules-lock/v1`、`final-report-index/v1`、`run-lock-packet/v1`、`bug-search-packet/v1`、`culprit-score/v1`、`verification-summary/v1`、`cleanup-plan/v1`、`handoff-next-step/v1` 或 compact packet/path/hash-first 優化，但讓它們取代 `.opencode/project-rules.md`、development-detail-planner、dispatch ledger、runner event、final maintained report、archive final file、commit map、verification log/source artifact、cleanup confirmation 或 branch contained gate，或允許 missing/stale/schema 不符/blocked 摘要通過 gate => `不一致`。
...
- 草稿/流程若使用 `run-preflight-packet/v1`、`verification-matrix/v1`、`package-decision-record/v1`、`experience-contract/v1` 或 `context-slice/v1`，但讓它們取代完整 development-detail-planner、project rules、Stage Execution Graph、Package Decision Record、Experience Contract、dispatch ledger、runner event、verification source 或 final maintained report，或允許 missing/stale/blocked/hash mismatch 摘要通過 gate => `不一致`。
- 草稿/流程未規劃 runner 只執行 Affected Verification Matrix 中本 classification 的必要 checks，merge 執行 integration checks，final 執行 final-only checks，而是讓每個 runner 重複跑全量驗證或把 runner-local checks 當 integration/final passed => `不一致`。
- 草稿/流程未規劃 project rules / active skill checkpoint memoization，或 hash 未變仍要求重貼全文作為預設 prompt/output，造成 token 重複且無品質收益 => `不一致`。
- 草稿/流程若使用 `openspec-template-contract/v1` 或 `apply-readiness-checklist/v1`，但讓它們取代 OpenSpec CLI、strict validate、alignment-check 或 `規格：...` commit，或缺 missing/stale/blocked fallback => `不一致`。
- 草稿/流程未規劃 `snapshot-manifest/v1` 增量同步，仍要求每個 worktree 預設完整 bulk copy root、重複計算 snapshot plan 或複製未被本 run 引用的大型 docs/assets/generated files => `不一致`。
- 草稿/流程未規劃 `commit-metadata-summary/v1`，導致 final report 必須預設掃描所有 commits/touched files，或 summary 被用來隱藏 commit requirement alignment 缺失 => `不一致`。
- 草稿/流程未規劃 schema validation scope：runner 驗證自身 artifacts、barrier 驗證 current wave、final 驗證 whole run，而是每階段無條件掃描全 run artifacts => `不一致`。
...
- 草稿/流程若列出的 summary/index/lock/cursor artifact 與 `init-project` artifact schema registry 命名不一致、漏列已使用的 `cleanup-locks/v1` / `classification-compact/v1` / `handoff-next-step/v1`、新增 `work/<run_id>/*` 類 alias、或 compact output 缺 `status/blockers/commits/verification/contextRefs/artifactRefs/nextAction/fallbackUsed` 任一欄位且非 final maintained report/archive final file => `不一致`。
