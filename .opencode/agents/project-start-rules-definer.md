...
- 規則中若包含 multi-worktree/OpenSpec staged flow，必須寫入長期派工契約：worktree-splitter stage-scoped；分類需輸出 `parallelGroupId`、`eligibleSetId`、`touchSet`、`contractInputs`、`contractOutputs`、`conflictRisk` 與 Stage Execution Graph；主流程依 `stage + lane + priority + parallelGroupId` eligible set 同批平行呼叫 runner，並用 dispatch ledger 記錄批次、結果與重試；runner 永遠只處理單一 worktree 且不得 merge upstream；下一 stage 必須以上一 stage integration 作為 baseline。
