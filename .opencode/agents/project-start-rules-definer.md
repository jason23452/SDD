...
- 規則中若包含 multi-worktree/OpenSpec staged flow，必須寫入長期派工契約：依 Stage Execution Graph 分 stage 建立/sync worktree；Stage N 只能在 Stage N-1 integration 完成後建立；propose-spec 與 apply-change 使用同一 canonical `eligibleSetId`；分類需輸出 `parallelGroupId`、`eligibleSetId`、`touchSet`、`contractInputs`、`contractOutputs`、`conflictRisk` 與 Stage Execution Graph；同一 eligibleSetId 多個 worktree 必須同輪平行呼叫 runner；dispatch ledger 記錄批次、結果與重試；runner 永遠只處理單一 worktree 且不得 merge upstream。
