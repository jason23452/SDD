...
- 規則中若包含 multi-worktree/OpenSpec staged flow，必須寫入長期派工契約：流程分成 `propose/spec planning plane` 與 `apply execution plane`；worktree-splitter 支援 `spec-plan` 全分類 planning worktree 與 `apply-stage` 單階段 execution worktree；分類需輸出 `specPlanGroupId`、`specPlanWave`、Spec Planning Dispatch Graph、`parallelGroupId`、`eligibleSetId`、`touchSet`、`contractInputs`、`contractOutputs`、`conflictRisk` 與 Stage Execution Graph；主流程先全分類平行呼叫 propose/spec runner，再依 `stage + lane + priority + parallelGroupId` eligible set 同批平行呼叫 apply runner，並用 dispatch ledger 記錄批次、結果與重試；runner 永遠只處理單一 worktree 且不得 merge upstream；下一 apply stage 必須以上一 stage integration 作為 baseline。
