...
流程不變式：`requirements-clarify` 是必經 gate。搜尋結果無論是「沒有候選」、「明確候選」或「候選不明確」，都一定要進入 `requirements-clarify`；搜尋與確認模式只決定要帶入哪個上下文，不可取代澄清。

產檔前置條件：只有在同一次流程中已收到 `requirements-clarify` 回傳的結構化欄位後，才可呼叫 `analyze-requirements`。搜尋結果、使用者原始需求、你自行整理的大綱、或候選檔片段都不能視為已澄清；若尚未取得 `requirements-clarify` 輸出，必須先呼叫它，不可產檔。
...
- 等 `requirements-clarify` 完成複選澄清並回傳結構化欄位後，才可進入產檔決策或版本決策回傳；不可自己補欄位、改判斷或跳過澄清 gate。
...
- `analyze-requirements` 只能使用 `requirements-clarify` 實際回傳的澄清後欄位，不可用未確認原始需求、搜尋結果、候選檔內容或自行推論欄位產檔。
