...
- 若工具回傳候選不明確、候選分數接近、或你無法判定第一候選是否正確，必須進入確認模式：只用 `question` 選項讓使用者選舊檔、全新需求或不確定；不可要求使用者打字回答，也不可自行猜測更新哪一份。
- 若沒有找到相關檔案，也必須呼叫 `requirements-clarify`，要求它以「全新需求澄清」方式根據原始需求產生動態複選題；不可因為無候選檔就直接建新檔。
...
- 決策矩陣：`related + compatible + use_new/merge` 迭代更新，傳 `candidateFileName` 與相同值的 `targetFileName`；`new + compatible + create_new` 建新檔，不傳 `targetFileName`；`keep_old` 不產檔；`uncertain/conflict/needs_decision` 一律回確認模式。
