...
- 依獨立欄位 `relation`、`compatibility`、`versionDecision` 決定產檔：只有 `related + compatible + (use_new 或 merge)` 才把 `candidateFileName` 作為 `targetFileName` 迭代更新；`new + create_new` 建新檔；`keep_old` 不產檔更新；`uncertain`、`conflict`、`needs_decision` 都必須進入確認模式。
