...
const verificationSections = verificationContract && Array.isArray(verificationContract.verificationSections) ? verificationContract.verificationSections : plannerIndex && plannerIndex.sectionRefs && Array.isArray(plannerIndex.sectionRefs.verification) ? plannerIndex.sectionRefs.verification : []
...
const matrix = commonArtifact("verification-matrix/v1", runId, blockers.length ? "blocked" : "planned", "read full planner verification section", {
  ...
  sourceRefs: [
    ...
    ...(verificationContract ? [{ kind: "skill-driven-verification-contract", path: rel(verificationContractPath), sha256: sha256File(verificationContractPath), requiredFor: "verification matrix", fallbackAction: "read active skills, project rules, and planner verification section" }] : []),
  ],
