...
- 只要建立、初始化、scaffold、修復或重建受本 skill 管轄的 E2E 測試基底，且 repo 原本不存在可沿用的 Playwright 契約，本 skill 內所有安裝、設定檔、browser install、scripts、base URL、test directory、CI/local verification 條款都屬 bootstrap 階段硬要求，必須在 bootstrap 一次完成。
- 不得把本 skill 的 bootstrap 條款標記為 `optional`、`deferred`、`pending`、`placeholder`、`later` 或任何等價延後語意；也不得先建立空測試目錄或未可執行設定就宣稱測試基底已完成。
