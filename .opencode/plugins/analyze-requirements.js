...
function formatAnalysis(analysis, projectSignals, requirementText) {
  ...
  return [
    ...
    "- 執行方式 question 只確認 frontend/backend/兩者/暫不初始化；一旦選擇初始化、建立、啟動或落地，預設自動續行完整 downstream：bootstrap -> development-detail-planner -> 主工作區 OpenSpec propose/spec -> 主工作區 apply-change/fallback -> integration verification，不再另問 downstream 授權題。",
    ...
    "- alignment 全通過後主流程在同一工作區執行 apply-change/fallback；OpenSpec apply blocked/失敗但 alignment 已通過時，必須 fallback 依 spec-flow artifacts 完成開發任務、驗證與中文細分 commit。",
...
function buildQuestionFreedomLines(analysis) {
  ...
  return [
    ...
    "- 最後問執行方式：frontend、backend、frontend + backend、暫不初始化；有 README=沿用，無 README=最小啟動建立；選擇初始化/建立/啟動/落地後預設自動走完整 downstream：bootstrap -> development-detail-planner -> 主工作區 OpenSpec propose/spec -> 主工作區 apply-change/fallback -> integration verification，不另問 downstream 授權題。",
  ]
}
