...
function formatAnalysis(analysis, projectSignals, requirementText) {
  ...
  return [
    ...
    "- project-bootstrapper 完成後回主流程產/更新 development-detail-planner，依分類 apply 階段、優先度 lane、執行優先度、parallelGroupId、eligibleSetId、touchSet 與 contract metadata 產 Stage Execution Graph，分 stage 交 worktree-splitter 建立 .worktree/<run_id>/stage-<n>/<name> 或等價 worktree，不能在啟動結果後中斷；只有使用者主動明確要求停止，才可限制 downstream。",
    "- worktree-splitter 依目前 apply stage 的通用需求分類、parallelGroupId、eligibleSetId 與優先度 lane 建立 worktree；stage 1 用 bootstrap/main baseline，stage N 必須等 stage N-1 integration 完成後以該 integration 結果建立/同步。每個 worktree 先從目前 SDD repository root bulk copy 可用開發 snapshot，但排除 .git/.worktree/spec-flow/.opencode/skills/.opencode/node_modules/.opencode/run/.opencode/local-docs/.opencode/outputs/.opencode/run-artifacts、node_modules、.venv、dist、cache、coverage、測試報告、local secrets、log/tmp/local DB；當前 run_id 文件與 dispatch ledger 再明確同步。",
    "- 每個 worktree 必須在自己的 spec-flow/ 初始化/產 proposal/specs/design/tasks，產 alignment-check；同一 apply 階段全通過才可進入該階段 apply；主流程依 Stage Execution Graph eligibleSetId 同輪平行 dispatch runner，不得依表格順序任意序列化。",
    "- 同一 apply 階段 alignment 全通過後，沿用 Stage Execution Graph eligible set 由主流程平行呼叫多個 runner subagent 執行 apply-change/fallback；eligible set 由 stage + lane + priority + parallelGroupId 組成，並以 eligibleSetId 與 dispatch ledger 追蹤。OpenSpec apply blocked/失敗但 alignment 已通過時，必須 fallback 依該 worktree 的 spec-flow artifacts 完成開發任務、驗證，並依 commit 授權狀態中文細分 commit；runner 不得 merge upstream integration。",
...
function buildQuestionFreedomLines(analysis) {
  ...
  return [
    ...
    "- 最後問執行方式：frontend、backend、frontend + backend、暫不初始化；有 README=沿用，無 README=最小啟動建立；選擇初始化/建立/啟動/落地後預設自動走完整 downstream：bootstrap -> development-detail-planner -> stage-scoped worktree-splitter -> 依 Stage Execution Graph eligibleSetId 分批 OpenSpec propose/spec -> 依相同 eligibleSetId 分批 apply-change/fallback -> stage/final merge integration；同一 eligible set 內多個 worktree 由主流程同一輪平行呼叫多個 runner subagent，並用 dispatch ledger 追蹤批次、錯誤與重試；不另問 downstream 或 commit 授權題。",
  ]
}
